(function () {
  ...
  async function postScan(code) {
    const csrf = (window.__csrf || getCookie('csrf_token') || '');
    const r = await fetch('/api/method/qr_scanner.api.create_scan', {
      method: 'POST',
      credentials: 'include', // login cookie'lerini gönder
      headers: {
    ...
    let j;
    try { j = await r.json(); }
    catch (e) { j = { ok: false, status: r.status, text: await r.text() }; }
