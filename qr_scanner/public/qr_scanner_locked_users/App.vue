...
const unlockUser = async (user: string) => {
	...
	try {
		...
		if (res.message && res.message.ok) {
			...
			frappe.show_alert({
				message: res.message?.msg || "Verification failed",
				indicator: "red",
			});
		}
	} catch (err) {
...
<template>
	<div class="locked-users-container" style="max-width: 800px; margin: 0 auto; padding: 20px">
		...
		<div
			v-else-if="lockedUsers.length === 0"
			class="empty-state"
			style="
				text-align: center;
				padding: 60px 20px;
		...
		<div v-else class="locked-list">
			...
			>
				<div class="user-info" style="display: flex; flex-direction: column">
					<span style="font-weight: 600; color: var(--text-color); font-size: 14px">{{
						user.full_name
					}}</span>
