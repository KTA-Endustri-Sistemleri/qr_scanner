...
<template>
	...
	<div
		...
		aria-hidden="true"
		:style="{
			display: props.mode === 'loading' ? 'block' : 'none',
			pointerEvents: props.mode === 'loading' ? 'all' : 'none',
		}"
	>
		<div class="qr-overlay-content" aria-live="polite">
			<div class="qr-spinner" aria-hidden="true"></div>
			<div>Processing…</div>
			<div class="qr-help" style="color: #fff; opacity: 0.9">
				Submitting record, please wait.
			</div>
		</div>
	</div>
	...
	<div
		...
		aria-hidden="true"
		:style="{
			display: props.mode === 'success' ? 'block' : 'none',
			pointerEvents: props.mode === 'success' ? 'all' : 'none',
		}"
	>
		<div class="qr-overlay-content" aria-live="polite">
			<div class="qr-dot-ok" aria-hidden="true"></div>
			<div>Saved</div>
			<div id="successDesc" class="qr-help" style="color: #fff; opacity: 0.9">
	...
	<div
		...
		aria-hidden="true"
		:style="{
			display: props.mode === 'warning' ? 'block' : 'none',
			pointerEvents: props.mode === 'warning' ? 'all' : 'none',
		}"
	>
		<div class="qr-overlay-content" aria-live="polite">
			<div class="qr-dot-ok" aria-hidden="true"></div>
			<div>Warning</div>
			<div id="warnDesc" class="qr-help" style="color: #fff; opacity: 0.9">
				{{ props.warnMsg }}
			</div>
		</div>
	</div>
...
.qr-overlay.loading .qr-spinner {
	...
	border-radius: 50%;
	animation: qrs 0.8s linear infinite;
}
