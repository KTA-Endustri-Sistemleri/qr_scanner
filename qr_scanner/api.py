...
def _get_settings_password():
	...
	try:
		pwd = get_decrypted_password(
			"QR Scan Settings", "QR Scan Settings", "unlock_password", raise_exception=False
		)
		if pwd:
			return str(pwd)
	except Exception:
...
def remote_unlock(target_user: str):
	...
	frappe.publish_realtime(
		event="qr_unlock_remote", message={"user": target_user, "status": "unlocked"}, user=target_user
	)
...
def get_locked_users():
	...
	users = frappe.get_all("User", filters={"custom_qr_locked": 1}, fields=["name", "full_name"])
	return users
...
def create_scan(
	...
	if len(qr_code) != 33:
		return {
			...
			"msg": _("QR code must be exactly 33 characters."),
		}
	...
	if is_locked:
		return {
			...
			"msg": _("A duplicate was previously scanned. Please unlock first."),
		}
	...
	# Yeni UNIQUE SQL hatası yakalama noktası
		...
		return {
			...
			"created": False,
			"reason": "duplicate",
			"msg": "Race Condition Engellendi, Aynı kayıttı.",
		}
